<?php
function ktlc_breadcrumbs( $id = null ) {
     ...
     * If not front page
     */
    $output = '<a href="' . site_url() . '">Home</a>' . $separator;

    /**
     * Post type is "___CUSTOM___"
     */
    // if ( is_single() && '___CUSTOM___' == get_post_type() ) {
    ...
    if ( is_single() ) {
        ...
        foreach ( $post_terms as $term ) {
            $output .= '<a href="' . get_term_link( $term ) . '" title="Go to ' . esc_attr( $term->name ) .  '">' . $term->name . '</a>' . $separator;
        }
